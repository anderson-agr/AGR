        ...
        private void Init()
        {
            ...
            Task.Run(async () => {
                while (true && this._Token.Token.IsCancellationRequested == false)
                {
                    ...
                    if (this._HasEnded)
                    {
                        this.ParentElement.State = AGR.Library.VideoState.ENDED;
                        this.ParentElement.Info = new VideoData
                        {
                            State = AGR.Library.VideoState.ENDED,
                    ...
                    else if (this.ParentElement.State != VideoState.ENDED)
                    {
                        if (this.IsPlaying)
                        {
                            this.ParentElement.State = AGR.Library.VideoState.PLAY;
                        }
                        else if (this.ParentElement.State != VideoState.PAUSE)
                        {
                            this.ParentElement.State = AGR.Library.VideoState.STOP;
                        }
